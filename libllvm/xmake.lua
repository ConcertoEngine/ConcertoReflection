package("libllvm")
    ...
    on_load(function (package)
        ...
        if package:is_plat("windows") then
            ...
            package:add("syslinks", "version", "ntdll")
        end
    ...
    on_install(function (package)
        ...
        if package:is_plat("windows") then
            table.insert(configs, "-DCMAKE_C_FLAGS=/FS")
            ...
            if dep and not dep:is_system() then
                ...
                if fetchinfo then
                    ...
                    if includedirs and #includedirs > 0 then
                        local incs = {}
                        for _, p in ipairs(includedirs) do
                            table.insert(incs, (p:gsub("\\", "/")))
                        end
                        table.insert(configs, "-D" .. varname .. "_INCLUDE_DIR=" .. "\"" .. table.concat(incs, ";") .. "\"")
                    end
                    local libfiles = fetchinfo.libfiles
                    if libfiles and #libfiles > 0 then
                        -- CMake attend typiquement un chemin de fichier unique ici
                        local libfile = libfiles[1]:gsub("\\", "/")
                        table.insert(configs, "-D" .. varname .. "_LIBRARY=" .. "\"" .. libfile .. "\"")
                    end
                end
            end
        end
        tryadd_dep("zlib", "ZLIB")
        tryadd_dep("zstd", "ZSTD")
