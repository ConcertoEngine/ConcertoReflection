...
parseClass (Element (QName "class" Nothing Nothing) attrs elems line) = do
    ...
    let autoGeneratedAttr = lookupAttr (unqual "autoGenerated") attrs
    ...
    let name = maybe (Left $ errorMsg "name") Right nameAttr
    let base = maybe (Right "") Right baseAttr
    let autoGenerated = maybe (Right False) (Right . (== "true")) autoGeneratedAttr

    case (name, autoGenerated) of
        (Right n, Right a) ->
            if null errors then
                if a && not (null methods) then
                    Just (Left ("autoGenerated class cannot have methods on line: " ++ maybe "X" show line ++ "\n"))
                else
                    Just (Right Class { className = n, classBase = either id id base, classAutoGenerated = a, classMethods = methods, classMembers = members })
            else
                Just (Left (concat errors ++ "\n"))
        _ -> Just (Left (either id id name ++ either id show autoGenerated ++ "\n"))
