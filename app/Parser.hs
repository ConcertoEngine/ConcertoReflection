...
parseClass (Element (QName "class" Nothing Nothing) attrs elems line) = do
    ...
    case (name, autoGenerated, base) of
        (Right n, Right a, Right b) ->
            if null errors then
                if a && not (null methods) then
                    Just (Left ("autoGenerated class cannot have methods on line: " ++ maybe "X" show line ++ "\n"))
                else
                    Just (Right Class { className = n, classBase = b, classAutoGenerated = a, classMethods = methods, classMembers = members })
            else
                Just (Left (concat errors ++ "\n"))
        _ -> Just (Left (either id id name ++ either id show autoGenerated ++ show base ++ "\n"))
