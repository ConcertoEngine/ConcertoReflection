...
parseInclude (Element (QName "include" Nothing Nothing) attrs _ line) = do
    ...
    let errorMsg attr = "<include> missing '" ++ attr ++ "' attribute on line: " ++ show line
...
parseClassMember (Element (QName "member" Nothing Nothing) attrs _ line) = do
    ...
    let errorMsg attr = "<member> missing '" ++ attr ++ "' attribute on line: " ++ show line
...
parseParam (Element (QName "param" Nothing Nothing) attrs _ line) = do
    ...
    let errorMsg attr = "<param> missing '" ++ attr ++ "' attribute on line: " ++ show line
    let name = maybe (Left $ errorMsg "name") Right nameAttr
    let type' = maybe (Left $ errorMsg "type") Right typeAttr
...
parseMethod (Element (QName "method" Nothing Nothing) attrs elems line) = do
    ...
    let publicAttr = lookupAttr (unqual "public") attrs
    let (errors, parameters) = partitionEithers (mapMaybe Main.parseParam (onlyElems elems))

    let errorMsg attr = "<method> missing '" ++ attr ++ "' attribute on line: " ++ show line
    let name = maybe (Left $ errorMsg "name") Right nameAttr
...
parseClass (Element (QName "class" Nothing Nothing) attrs elems line) = do
    let nameAttr = lookupAttr (unqual "name") attrs
    let (methodErrors, methods) = partitionEithers (mapMaybe Main.parseMethod (onlyElems elems))
    let (memberErrors, members) = partitionEithers (mapMaybe Main.parseClassMember (onlyElems elems))
    let errors = methodErrors ++ memberErrors

    let errorMsg attr = "<class> missing '" ++ attr ++ "' attribute on line: " ++ show line
    let name = maybe (Left $ errorMsg "name") Right nameAttr
...
parseEnum (Element (QName "enum" Nothing Nothing) attrs elems line) = do
    ...
    let (errors, members) = partitionEithers (mapMaybe Main.parseEnumMember (onlyElems elems))

    let errorMsg attr = "<enum> missing '" ++ attr ++ "' attribute on line: " ++ show line
    let name = maybe (Left $ errorMsg "name") Right nameAttr
...
parseNamespace (Element (QName "namespace" Nothing Nothing) attrs elems _) = do
    ...
    let (enumErrors, enumerations) = partitionEithers (mapMaybe Main.parseEnum (onlyElems elems))
    let (klassErros, klasses) = partitionEithers (mapMaybe Main.parseClass (onlyElems elems))
...
parsePackage (Element (QName "package" Nothing Nothing) attrs contents _) = do
    ...
    let (includeErrors, includes) = partitionEithers (mapMaybe Main.parseInclude elems)
    let (namespaceErrors, namespaces) = partitionEithers (mapMaybe Main.parseNamespace elems)
    let (classErrors, classes) = partitionEithers (mapMaybe Main.parseClass elems)
    let (enumErrors, enums) = partitionEithers (mapMaybe Main.parseEnum elems)
